...
export class InventoryRepository {
  ...
  async getInventory(filters: { userId?: string; itemId?: string; sellable?: boolean; purchasePrice?: number; uniqueId?: string; minAmount?: number } = {}): Promise<InventoryItem[]> {
    ...
    const items = await db.collection('inventories').aggregate(pipeline).toArray() as unknown as InventoryItem[];
    return items;
  }
  ...
  async setItemAmount(userId: string, itemId: string, amount: number): Promise<void> {
    ...
    const result = await db.collection('inventories').updateOne(
      { user_id: userId, item_id: itemId, metadata: null },
      { $set: { amount } }
    );
    if (result.matchedCount === 0) {
      await db.collection('inventories').insertOne({
  ...
  async updateItemMetadata(userId: string, itemId: string, uniqueId: string, metadata: object): Promise<void> {
    ...
    await db.collection('inventories').updateOne(
      { user_id: userId, item_id: itemId, 'metadata._unique_id': uniqueId },
      { $set: { metadata: metadataWithUniqueId } }
    );
  }
