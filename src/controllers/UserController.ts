...
export class Users {
  ...
  private mapUser(user: User) {
    return {
      ...
      email: user.email,
      balance:
        user.balance !== undefined ? Math.floor(user.balance) : undefined,
  ...
  public async loginOAuth(req: Request, res: Response) {
    ...
    if (provider === "discord") {
      ...
      if (!redirectUri) {
        ...
        return this.sendError(
          res,
          500,
          "Discord redirect_uri is not set in environment variables",
        );
      }
      ...
      if (!tokenRes.ok) {
        ...
        return this.sendError(
          res,
          500,
          "Failed to fetch Discord access token: " + errorText,
        );
      }
    ...
    if (!user) {
      ...
      if (
        (provider === "discord" &&
          user.discord_id &&
          user.discord_id != verifiedUser.id) ||
        (provider === "google" &&
          user.google_id &&
          user.google_id != verifiedUser.id)
      ) {
  ...
  async getMe(req: AuthenticatedRequest, res: Response) {
    ...
    const userWithData =
      await this.userService.getUserWithCompleteProfile(userId);
    if (!userWithData) {
    ...
    const studios = await this.studioService.getUserStudios(
      req.originalUser?.user_id || userId,
    );
    const roles = [
  ...
  public async reenableAccount(req: AuthenticatedRequest, res: Response) {
    ...
    if (adminUserId === userId) {
      ...
      return res
        .status(400)
        .send({ message: "Vous ne pouvez pas réactiver votre propre compte." });
    }
    try {
