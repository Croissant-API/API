...
export interface IUserService {
	...
	getUserWithCompleteProfile(
		user_id: string,
	): Promise<(User & UserExtensions) | null>;
	getUserWithPublicProfile(
		user_id: string,
	): Promise<(PublicUser & UserExtensions) | null>;
	adminGetUserWithProfile(
		user_id: string,
...
export class UserService implements IUserService {
	...
	private static getIdWhereClause(includeDisabled = false) {
		const base =
			"(user_id = ? OR discord_id = ? OR google_id = ? OR steam_id = ?)";
		if (includeDisabled) return base;
		return base + " AND (disabled = 0 OR disabled IS NULL)";
	}

	private async fetchUserByAnyId(
		user_id: string,
	...
	private async updateUserFields(
		user_id: string,
		fields: Partial<Pick<User, "username" | "balance" | "password">>,
	): Promise<void> {
	...
	async authenticateUser(tokenOrApiKey: string): Promise<User | null> {
		...
		const user =
			users.find((user) => genKey(user.user_id) === apiKey) || null;
		if (!user) {
			return null;
		}
		return user;
	...
	async getUserWithCompleteProfile(
		user_id: string,
	): Promise<(User & UserExtensions) | null> {
		const query = `
      SELECT 
    ...
    `;
       ...
       AND item_id NOT IN (
		...
		const results = await this.databaseService.read<User & UserExtensions>(
			query,
			[user_id, user_id, user_id, user_id],
		);
