...
export class StudioService implements IStudioService {
    ...
    async setStudioProperties(user_id: string, admin_id: string, users: User[]): Promise<void> {
    ...
    async getUserStudios(user_id: string): Promise<Array<Studio & { isAdmin: boolean }>> {
