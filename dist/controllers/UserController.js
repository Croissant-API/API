...
let Users = class Users {
    ...
    getOriginalUserFromContext(c) {
        return c.get('originalUser');
    }
    async loginOAuth(c) {
        try {
    ...
    async register(c) {
        try {
            ...
            const user = await this.userService.createUser(userId, body.username, body.email, hashedPassword, body.provider, body.providerId);
    ...
    async getMe(c) {
        try {
            ...
            const roles = [originalUser?.user_id, ...studios.map(s => s.user_id)];
...
Users = __decorate([
    ...
    __param(2, inject('MailService')),
    __param(3, inject('StudioService')),
    __param(4, inject('SteamOAuthService')),
    __metadata("design:paramtypes", [Object, Object, MailService,
        StudioService,
        SteamOAuthService])
], Users);
export { Users };
