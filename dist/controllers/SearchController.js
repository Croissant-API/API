...
let SearchController = class SearchController {
    ...
    async handleSearch(req, res, { admin = false, userId } = {}) {
        ...
        try {
            ...
            const games = (await this.gameService.listGames()).filter(g => g.showInStore && [g.name, g.description, g.genre].some(v => v && v.toLowerCase().includes(query.toLowerCase()))).map(game => (0, helpers_1.filterGame)(game));
