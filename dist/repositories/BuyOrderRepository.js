...
class BuyOrderRepository {
    ...
    async updateBuyOrderStatusToCancelled(orderId, buyerId, updatedAt) {
        ...
        const db = await this.databaseService.getDb();
        await db.collection('buy_orders').updateOne({ id: orderId, buyer_id: buyerId, status: 'active' }, { $set: { status: 'cancelled', updated_at: new Date(updatedAt) } });
    }
    async getBuyOrders(filters = {}, orderBy = 'created_at DESC', limit) {
        ...
        const buyOrders = result.map(doc => ({
            id: doc.id,
