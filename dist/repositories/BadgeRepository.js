export class BadgeRepository {
    ...
    async getActiveBadgesForGame(gameId) {
      ...
      WHERE gb.game_id = ? AND gb.expires_at > CURRENT_TIMESTAMP
    ...
    async addBadgeToGame(gameId, badgeId, durationDays) {
       ...
       created_at = CURRENT_TIMESTAMP, expires_at = ?`, [gameId, badgeId, expiresAt, expiresAt]);
    }
    async removeExpiredBadges() {
        await this.databaseService.request('DELETE FROM game_badges WHERE expires_at < CURRENT_TIMESTAMP');
    }
