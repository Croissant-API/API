...
class BadgeRepository {
    ...
    async getBadgeTypes() {
        ...
        const badgeTypes = result.map(doc => ({
            id: doc.id,
            name: doc.name,
    ...
    async addBadgeToGame(gameId, badgeId, durationDays) {
        ...
        const db = await this.databaseService.getDb();
        await db.collection('game_badges').updateOne({ gameId: gameId, badgeId: badgeId }, { $set: { created_at: new Date(), expires_at: expiresAt } }, { upsert: true });
    }
