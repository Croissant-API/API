...
export function findUserByResetToken(users, reset_token) {
...
export function mapUserSearch(user) {
    return {
        id: user.user_id,
        userId: user.user_id,
        username: user.username,
...
export function filterGame(game, userId, myId) {
    return {
        gameId: game.gameId,
        name: game.name,
...
export async function formatInventory(inventory, itemService) {
    const seen = new Set();
