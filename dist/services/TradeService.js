...
let TradeService = class TradeService {
    ...
    async addItemToTrade(tradeId, userId, tradeItem) {
        ...
        if (tradeItem.metadata?._unique_id) {
            const uniqueClause = this.databaseService.isPostgres && this.databaseService.isPostgres()
                ? "metadata->>'_unique_id' = ?"
         ...
         WHERE user_id = ? AND item_id = ? AND ${uniqueClause}`, [userId, tradeItem.itemId, tradeItem.metadata._unique_id]);
